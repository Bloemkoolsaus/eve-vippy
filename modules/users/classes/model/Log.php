...
class Log
{
     ...
     * @return Log[]
     */
    public static function getLogByUserOnDate($userID, $sdate, $edate, $what=null, $whatID=null)
    {
        $query = [];
        if ($what)
            $query[] = "what = '".\MySQL::escape($what)."'";
        if ($whatID && is_numeric($whatID))
            $query[] = "whatid = ".$whatID;
        ...
        if ($results = \MySQL::getDB()->getRows("select *
                                                ...
                                                ".((count($query)>0)?" and ".implode(" and ", $query):"")."
                                                order by logdate desc"
                , [$userID, date("Y-m-d",strtotime($sdate))." 00:00:00", date("Y-m-d",strtotime($edate))." 23:59:59"]))
        {
            foreach ($results as $result)
            {
                $log = new \users\model\Log();
